...
sub set_attr
{
  ...
  while ( ( $key, $value ) = each %{$attr} )
  {
    ...
    {
      $self->_errmsg("unknown attribute: `$key'");
      return undef;
    }
    $self->{'attr'}{$key} = $value;
  }
...
sub AUTOLOAD {
	...
	$self->{'errmsg'} = "$keyword doesn't exist";
	return undef;
      }
    }
...
assumed to be and environmental variable.
...
=item B<new Config::Wild ([$config_file])>

Create a new B<Config::Wild> object, optionally loading configuration information
from a file.  It returns the new object, or C<undef> upon error.

=item B<load( $file )>

Load information from a configuration file into the current object.  
New configuration values will supersede previous ones, in a complicated
fashion.  Absolutely specified keywords will overwrite previously 
absolutely specified values.  Since it is difficult to determine
whether the set of keywords matched by two regular expressions overlap,
wildcard keywords are pushed onto a last-in first-out (LIFO) list,
so that when the application requests a value, it will use search
the wildcard keywords in reverse order that they were specified.

It returns 1 upon success, C<undef> if an error ocurred.  The
error message may be retrieved with the B<errmsg> method.

=item B<load_cmd( \@ARGV [,\%attr] )>

Parse an array of keyword-value pairs (possibly command line arguments),
and insert them into the list of keywords.  It can take an optional list of
attributes:
...
1, upon error it returns C<undef> and sets the objects error message
(see B<errmsg()>).
...
=item B<value( $keyword )>

Return the value associated with a given keyword.  B<$keyword> is first
matched against the absolute keywords, then agains the wildcards.
If no match is made, C<undef> is returned.

=item B<delete( $keyword )>
...
It should return a value, which will be returned as the value of
the keyword.

For example,

  $cfg = Config::Wild->("foo.cnf");
...
If true, all errors will result in a call to B<warn()>.  If it
is set to a reference to a function, that function will be called
instead.
...
keywords.  You may access a keyword's value by specifying the
keyword as the method, instead of using B<value>.  The following
are equivalent:

   $foo = $cfg->value( 'keyword' );
   $foo = $cfg->keyword;

If C<keyword> doesn't exist, it returns C<undef>.

Similarly, you can set a keyword's value using a similar syntax.
The following are equivalent, if the keyword already exists:
...
It is a bit more time consuming to use these methods rather than
using B<set> and B<value>.
...
B<AppConfig> and early version of which was the inspiration for this
module.


=head1 AUTHOR

Diab Jerius, E<lt>dj@cpan.orgE<gt>
